...
for this package (i.e., help(RGMQL)).
...
# Granges Object with only one region: chr2 and two metadata column: score = 5 
# and GC  = 0.45

gr1 <- GRanges(seqnames = "chr2",
    ranges = IRanges(103, 106), strand = "+", score = 5L, GC = 0.45)

# Granges Object with two regions both chr1 and two metadata column: score = 3
...
collect(exon_res)
```
by default *collect()* has R working directory as stored path and *ds1* as name 
...
- Batch execution: \newline
    ...
    uploads them on the remote system; once loaded, RGMQL functions can be 
...
or can execute a query reading directly from file (e.g., the "query1.txt" 
file in the "example" subdirectory of the RGMQL package):
...
the first can be materialized with:   
...
HG19_bed_ann <- read_gmql("public.Example_Dataset_1", is_local = FALSE)
...
# Import the GMQL dataset as GRangesList
...
# Export the GRangesList as GMQL dataset called 'example' at destination path
