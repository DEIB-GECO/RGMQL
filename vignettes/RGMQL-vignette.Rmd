...
Eache sample of GMQL dataset has its own metadata associated and generally 
every metadata has a single value. In case we got mutiple distinct values 
for the same metadata-key attributes, as showed in the figure below:  
\newline
![metadata with multiple values](multi_metadata.png)
\newline\newline
In this case we have multiple values for metadata *disease*; GMQL auomcatically
handle this 'issue'.
In the import/export paragrapgh we showed that the GMQL dataset can be 
imported and so its metadata too.
...
```
The metadata is stored as simple list in the form key-values and it doesn't
matter if mutiple values for one metadata-key attribute are present; all values
are stored and showed.
The problem is raised once we need to get the metadata value; normally since 
listis in the form key-value we can extract the metadata value using:

```{r, retrieve_value}

# stored metadata on variable a
...
```
In this case only the first value is showed. If we would retrieve all disease, 
we should use:
